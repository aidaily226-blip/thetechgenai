...
export default function About() {
  return (
    ...
    >
      <style>{`
        .about-hero-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 3.5rem; align-items: center; }
        .about-stats-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; }
        .founder-grid { display: grid; grid-template-columns: 280px 1fr; gap: 3rem; align-items: start; }
        .skills-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }
        .what-we-do-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; margin-bottom: 2rem; }
        .values-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.25rem; }
        .pills-wrap { display: flex; flex-wrap: wrap; gap: 0.75rem; justify-content: center; }
        ...
        @media (max-width: 768px) {
          ...
          .what-we-do-grid { grid-template-columns: 1fr !important; }
      ...
      {/* Hero */}
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '3.5rem 1.5rem 3rem', position: 'relative' }}>
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: '600px', height: '300px', background: 'radial-gradient(ellipse, rgba(0,255,136,0.05) 0%, transparent 70%)', pointerEvents: 'none' }} />
        ...
        <div className="about-hero-grid">
          ...
          <div>
            ...
            <div className="about-cta-btns" style={{ justifyContent: 'flex-start' }}>
              <Link href="/contact"
          ...
          <div className="about-stats-grid">
            {[
              ...
              <div key={s.label} style={{ background: '#0d1117', border: '1px solid #1e2a38', borderRadius: '16px', padding: '1.5rem', textAlign: 'center' }}>
                <p style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 'clamp(1.5rem, 3vw, 2rem)', color: s.color, marginBottom: '0.25rem' }}>{s.number}</p>
                <p style={{ color: '#475569', fontSize: '0.75rem', lineHeight: 1.4 }}>{s.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <div className="founder-grid">
            ...
            <div style={{ background: '#080d14', border: '1px solid #1e2a38', borderRadius: '20px', padding: '2rem', textAlign: 'center' }}>
              <div style={{ width: '80px', height: '80px', background: 'linear-gradient(135deg, #0ea5e9, #00ff88)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem', fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.6rem', color: '#080d14' }}>
                PG
              </div>
      ...
      <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '3.5rem 1.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.5rem', color: '#f1f5f9', marginBottom: '0.5rem' }}>
            What We <span style={{ color: '#0ea5e9' }}>Do</span>
          </h2>
          <p style={{ color: '#475569', fontSize: '0.88rem' }}>Two businesses. One platform. Built to grow your business and keep you informed.</p>
        </div>
        <div className="what-we-do-grid">
          <div style={{ background: '#0d1117', border: '1px solid rgba(14,165,233,0.2)', borderRadius: '16px', padding: '1.75rem' }}>
            <div style={{ fontSize: '2.2rem', marginBottom: '0.85rem' }}>🏢</div>
            <h3 style={{ fontFamily: 'Syne, sans-serif', color: '#0ea5e9', fontWeight: 700, fontSize: '1.05rem', marginBottom: '0.65rem' }}>Digital Agency</h3>
            <p style={{ color: '#64748b', fontSize: '0.86rem', lineHeight: 1.7, marginBottom: '1rem' }}>
              We build AI-powered websites, handle digital marketing, design brand identities, and create e-commerce stores for businesses across India.
            </p>
            <Link href="/services" style={{ color: '#0ea5e9', fontSize: '0.82rem', fontWeight: 600, textDecoration: 'none' }}>View Services →</Link>
          </div>
          <div style={{ background: '#0d1117', border: '1px solid rgba(0,255,136,0.2)', borderRadius: '16px', padding: '1.75rem' }}>
            <div style={{ fontSize: '2.2rem', marginBottom: '0.85rem' }}>📰</div>
            <h3 style={{ fontFamily: 'Syne, sans-serif', color: '#00ff88', fontWeight: 700, fontSize: '1.05rem', marginBottom: '0.65rem' }}>Tech Blog</h3>
            <p style={{ color: '#64748b', fontSize: '0.86rem', lineHeight: 1.7, marginBottom: '1rem' }}>
              We publish daily articles on artificial intelligence, technology news, gadget reviews and strategies for earning money online — in simple words.
            </p>
            <Link href="/blog" style={{ color: '#00ff88', fontSize: '0.82rem', fontWeight: 600, textDecoration: 'none' }}>Read Blog →</Link>
          </div>
        </div>
        <div className="pills-wrap">
          {servicesPills.map(s => (
            <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: '#0d1117', border: '1px solid #1e2a38', borderRadius: '100px', padding: '0.4rem 1rem', color: '#64748b', fontSize: '0.8rem' }}>
      ...
      <section style={{ background: '#0d1117', borderTop: '1px solid #1e2a38', borderBottom: '1px solid #1e2a38', padding: '3.5rem 1.5rem' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.5rem', color: '#f1f5f9' }}>
          ...
          <div className="values-grid">
            {values.map(v => (
              <div key={v.title} style={{ background: '#080d14', border: '1px solid #1e2a38', borderRadius: '14px', padding: '1.5rem' }}>
                <div style={{ fontSize: '1.6rem', marginBottom: '0.65rem' }}>{v.icon}</div>
                <h3 style={{ fontFamily: 'Syne, sans-serif', color: '#f1f5f9', fontWeight: 700, fontSize: '0.92rem', marginBottom: '0.35rem' }}>{v.title}</h3>
                <p style={{ color: '#475569', fontSize: '0.8rem', lineHeight: 1.6 }}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section style={{ maxWidth: '600px', margin: '0 auto', padding: '4rem 1.5rem', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 'clamp(1.4rem, 4vw, 2rem)', color: '#f1f5f9', marginBottom: '1rem' }}>
          Ready to Work Together?
        </h2>
        <p style={{ color: '#64748b', marginBottom: '1.75rem', lineHeight: 1.7, fontSize: '0.92rem' }}>
        ...
        <div className="about-cta-btns">
          <a href="https://wa.me/919870405692" target="_blank" rel="noopener noreferrer"
            style={{ padding: '0.85rem 1.5rem', background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '8px', color: '#22c55e', fontWeight: 700, textDecoration: 'none' }}>
            💬 WhatsApp Now
          </a>
          <Link href="/contact"
            style={{ padding: '0.85rem 1.5rem', background: 'linear-gradient(135deg, #0ea5e9, #0284c7)', borderRadius: '8px', color: 'white', fontWeight: 700, textDecoration: 'none', fontFamily: 'Syne, sans-serif' }}>
            Get Free Quote →
          </Link>
        </div>
        <p style={{ color: '#334155', fontSize: '0.75rem', marginTop: '1.1rem' }}>
